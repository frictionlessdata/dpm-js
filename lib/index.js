...
Dpm.prototype.get = function(dpkgId, opts, callback) {
  ...
  var self = this
    , spec = ourutil.parseSpecString(dpkgId)
  ...
  request.get(dpjsonUrl, function(err, res, body) {
    ...
    async.each(resources, function(r, cb) {
      ...
      self.logHttp('GET', r.url);
      var req = request(r.url);
      req.on('error', cb);
      req.on('response', function(resp){            
        self.logHttp(resp.statusCode, r.url);
        ...
        if (r.path) {
          ...
          var filename = url.parse(url_).pathname.split('/').pop();
